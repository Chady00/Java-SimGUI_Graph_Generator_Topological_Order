...
class ThreeTenGraph implements Graph<GraphNode,GraphEdge>, DirectedGraph<GraphNode,GraphEdge> {
	...
	public ThreeTenGraph() {
		...
		this.vertexList= new GraphNode[200];
		this.matrix= new GraphEdge[200][200];
	}
	...
	public Collection<GraphNode> getVertices() {
		...
		// Loop through vertexList array and add each vertex to the collection
	...
	public boolean containsVertex(GraphNode vertex) {
		...
		//System.out.println("6");
                for(int i=0; i<vertexList.length;i++){
                    if(vertex == vertexList[i])return true;
                }
	...
	public Collection<GraphEdge> getInEdges(GraphNode vertex) {
		//System.out.println("indedges");
		List<GraphEdge> inedges = new ArrayList<>();

		for (int i = 0; i < matrix.length; i++) {
			GraphEdge e= matrix[i][vertex.getId()];
			if (e != null) {
				inedges.add(e);
			}
		}
	...
	public Collection<GraphEdge> getOutEdges(GraphNode vertex) {
		//System.out.println("outdeges");
		List<GraphEdge> outEdges = new ArrayList<>();
		for (int i = 0; i < matrix.length; i++) {
			GraphEdge e= matrix[vertex.getId()][i];
			if (e != null  ) {
				outEdges.add(e);
			}
		}
...
//        System.out.println("this is what the matrix looks like: \n");
//        for(int i=0; i<matrix.length;i++){
//            for(int j=0;j<matrix[i].length;j++){
//                if(matrix[i][j]!=null)
//                    System.out.println(" Source is: "+getSource(matrix[i][j]).getId() + " dest is: "+ getDest(matrix[i][j]).getId());
//            }
//        }
		//O(n) where n is the max number of vertices in the graph
		// System.out.println(vertexList.length);
		if(vertex !=null){
			//System.out.println(vertex.getId()+"..."+getOutEdges(vertex).size());
	...
	public Collection<GraphNode> getPredecessors(GraphNode vertex) {
		...
		for (GraphEdge edge : getInEdges(vertex)) {
	...
	public Collection<GraphNode> getSuccessors(GraphNode vertex) {
		...
		for (int j = 0; j < vertexList.length; j++) {
			if (matrix[i][j] != null) {
				successors.add(vertexList[j]);
			}
		}
		// print all successors
		for (GraphNode v : successors) {
			//System.out.println("successor: " + v.getId());
		}
		return successors;
	}
	...
	public boolean isSuccessor(GraphNode v1, GraphNode v2) {
		//O(1)
		 return matrix[v2.getId()][v1.getId()] != null;
	}
	...
	//	System.out.print("getneighborcount");
		return vertex!= null? getNeighbors(vertex).size() : -1;
	}
	...
	public GraphNode getSource(GraphEdge directed_edge) {
		...
		for (int i = 0; i < matrix.length; i++) {
			for (int j = 0; j < matrix[i].length; j++) {
				...
				if (e != null && e.equals(directed_edge)) {
					return vertexList[i];
				}
			}
		}
	...
	public boolean isIncident(GraphNode vertex, GraphEdge edge) {
        // Check if the edge has the given vertex as one of its endpoints
            return getSource(edge).equals(vertex) || getDest(edge).equals(vertex);
        }
	...
	public boolean addEdge(GraphEdge e, GraphNode v1, GraphNode v2) {
		// Find the indices of the two vertices in the matrix
//		System.out.print("addedge");
		...
		//   System.out.println(index1 + "goes to  " + index2);
		if(v1 == v2){
			System.out.println("you can't add an edge to itself");
			return false;
		}
		matrix[index1][index2] = e;
//		System.out.println(" I added edge with id "+ e.id + " between "+ index1 + " and " + index2);
...
//            for(int j=0;j<matrix[i].length;j++){
//                if(matrix[i][j]!=null)
//                    System.out.println(matrix[i][j].id);
//            }
//        }        }
		// matrix[index2][index1] = e; // If the graph is undirected, add the edge in both directions
...
//       if (vertexList == null) {
//        // If vertexList is null, create a new array with a size of 1 and add the vertex to it
//        vertexList = new GraphNode[1];
//        vertexList[0] = vertex;
//    } else {
//        // If vertexList is not null, create a new array with a size one greater than the current size of vertexList
//        GraphNode[] temp = vertexList;
//        vertexList = new GraphNode[vertexList.length + 1];
//        // Copy the existing elements from vertexList to the new array
//        for (int i = 0; i < temp.length; i++) {
//            vertexList[i] = temp[i];
//        }
//        // Add the new vertex to the end of the new array
...
//    return true;
	...
	public boolean removeEdge(GraphEdge edge) {
            ...
            GraphNode src = getSource(edge);
            GraphNode dest = getDest(edge);

            // Check if the source and destination vertices are present in the graph
            if (src.getId() == -1 || dest.getId() == -1) {
                return false;
            }

            // Set the corresponding entry in the matrix to null
            matrix[src.getId()][dest.getId()] = null;
	...
	public String depthFirstTraversal(GraphNode node) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (!visited.contains(current)) {
                ...
                for (GraphEdge edge : getOutEdges(current)) {
                    GraphNode neighbor = getOpposite(current, edge);
...
//		}
	...
	public boolean isNeighbor(GraphNode v1, GraphNode v2) {
		System.out.println("isneightbor");
	...
	public Collection<GraphEdge> getIncidentEdges(GraphNode vertex) {
		//System.out.println("getIncEdges");
	...
	public Collection<GraphNode> getIncidentVertices(GraphEdge edge) {
		//System.out.println("getIncidentVert");
	...
	public int getPredecessorCount(GraphNode vertex) {
		//System.out.println("getPredcesorCount");
	...
	public int getSuccessorCount(GraphNode vertex) {
		//System.out.println("getSuccesorCount");
	...
	//	System.out.println("findEdgeSet");
